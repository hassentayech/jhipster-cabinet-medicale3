...
import com.forticas.cabinet3.service.ReglementService;
...
/**
 * REST controller for managing {@link com.forticas.cabinet3.domain.Reglement}.
 */
@RestController
@RequestMapping("/api")
public class ReglementResource {
    ...
    public ResponseEntity<Reglement> updateReglement(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, reglement.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/reglements/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<Reglement> partialUpdateReglement(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, reglement.getId().toString())
        );
    }
