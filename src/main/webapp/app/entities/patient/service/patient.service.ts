...
export class PatientService {
  ...
  protected convertDateFromClient(patient: IPatient): IPatient {
    return Object.assign({}, patient, {
      naissance: patient.naissance?.isValid() ? patient.naissance.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.naissance = res.body.naissance ? dayjs(res.body.naissance) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((patient: IPatient) => {
        patient.naissance = patient.naissance ? dayjs(patient.naissance) : undefined;
      });
    }
    return res;
  }
